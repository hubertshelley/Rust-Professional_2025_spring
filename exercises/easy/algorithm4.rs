    ...
    T: Ord,
{
    ...
    fn search(&self, value: T) -> bool {
        match value.cmp(&self.value) {
            Ordering::Less => self.left.as_ref().is_some_and(|left| left.search(value)),
            Ordering::Equal => true,
            Ordering::Greater => self.right.as_ref().is_some_and(|right| right.search(value)),
        }
    }
}
